...
class DavWave : public DavProc {
   ...
   public: /* DavWave connection and disconnection */
    ...
    static int connect(DavWave *from, DavWave *to, int fromStreamIndex = -1) {
        ...
        string detail = "from stream " + std::to_string(fromStreamIndex) + " of " +
                        from->getLogTag() + " => to stream of " + to->getLogTag();
    ...
    static int disconnect(DavWave *from, DavWave *to, int fromStreamIndex = -1) {
        ...
        string detail = "from stream " + std::to_string(fromStreamIndex) + " of " +
                        from->getLogTag() + " <= disconnect => " + to->getLogTag();
    ...
    static int subscribeData(std::shared_ptr<DavWave> &from, std::shared_ptr<DavWave> to,
                             int fromStreamIndex = -1) {
        return connect(from.get(), to.get(), fromStreamIndex);
    }
    static int unsubscribeData(std::shared_ptr<DavWave> &from,
                               std::shared_ptr<DavWave> to, int fromStreamIndex = -1) {
        return disconnect(from.get(), to.get(), fromStreamIndex);
    }
   ...
   public: /* trivial helpers */
    ...
    map<int, AVMediaType> getOutputMediaMap() {
        if (m_impl) return m_impl->getOutputMediaMap();
        return {};
    }
