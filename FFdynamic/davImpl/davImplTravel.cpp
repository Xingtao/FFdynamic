...
ostream &operator<<(ostream &os, const DavTravelStatic &s) {
    ...
    if (s.m_mediaType == AVMEDIA_TYPE_VIDEO) {
        os << ", pixel format " << (int)s.m_pixfmt << ", timebase " << s.m_timebase
           << ", width " << s.m_width << ", height " << s.m_height << ", sar " << s.m_sar
           << ", framerate " << s.m_framerate << ", hwFramesctx " << s.m_hwFramesCtx;
...
int DavTravelStatic::mergeAudioDavTravelStaticToDict(DavWaveOption &options) {
    ...
    options.set("channel_layout", std::to_string(m_channelLayout),
...
int DavTravelStatic::setupVideoStatic(AVCodecContext *codecCtx,
                                      const AVRational &timebase,
                                      const AVRational &framerate,
                                      AVBufferRef *hwFramesCtx) noexcept {
...
int DavTravelStatic::setupVideoStatic(AVCodecParameters *codecpar,
                                      const AVRational &timebase,
                                      const AVRational &framerate,
                                      AVBufferRef *hwFramesCtx) noexcept {
    ...
    return setupVideoStatic((enum AVPixelFormat)codecpar->format, codecpar->width,
                            codecpar->height, timebase, framerate,
                            codecpar->sample_aspect_ratio, hwFramesCtx);
}

int DavTravelStatic::setupVideoStatic(enum AVPixelFormat pixfmt, int width, int height,
                                      const AVRational &timebase,
                                      const AVRational &framerate,
...
int DavTravelStatic::setupAudioStatic(AVCodecContext *codecCtx,
...
int DavTravelStatic::setupAudioStatic(AVCodecParameters *codecpar,
                                      const AVRational &timebase) noexcept {
    ...
    m_timebase = timebase;
    return setupAudioStatic((enum AVSampleFormat)codecpar->format, timebase,
                            codecpar->sample_rate, codecpar->channels,
                            codecpar->channel_layout);
}

int DavTravelStatic::setupAudioStatic(enum AVSampleFormat samplefmt,
                                      const AVRational &timebase, int samplerate,
                                      int channels, uint64_t channelLayout) noexcept {
    m_mediaType = AVMEDIA_TYPE_AUDIO;
