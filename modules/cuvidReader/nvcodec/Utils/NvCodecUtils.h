...
inline bool check(CUresult e, int iLine, const char *szFile) {
    if (e != CUDA_SUCCESS) {
        ...
        std::cerr << "CUDA driver API error " << szErrName << " at line " << iLine << " in file " << szFile;
        return false;
    }
    return true;
}
...
inline bool check(cudaError_t e, int iLine, const char *szFile) {
    if (e != cudaSuccess) {
        std::cerr << "CUDA runtime API error " << cudaGetErrorName(e) << " at line " << iLine << " in file " << szFile;
        return false;
    }
    return true;
}
...
inline bool check(NVENCSTATUS e, int iLine, const char *szFile) {
    ...
    if (e != NV_ENC_SUCCESS) {
        std::cerr << "NVENC error " << aszErrName[e] << " at line " << iLine << " in file " << szFile;
        return false;
    }
    return true;
}
...
inline bool check(HRESULT e, int iLine, const char *szFile) {
    if (e != S_OK) {
        std::cerr << "HRESULT error 0x" << (void *)e << " at line " << iLine << " in file " << szFile;
        return false;
    }
    return true;
}
...
inline bool check(GLenum e, int iLine, const char *szFile) {
    if (e != 0) {
        std::cerr << "GLenum error " << e << " at line " << iLine << " in file " << szFile;
        return false;
    }
    return true;
}
    ...
    BufferedFileReader(const char *szFileName, bool bPartial = false) {
        ...
        while (nSize) {
            try {
                ...
                if (nSize != st.st_size) {
                  std::cout << "File is too large - only " << std::setprecision(4) << 100.0 * nSize / (uint32_t)st.st_size << "% is loaded";
                }
                break;
            } catch(std::bad_alloc) {
                if (!bPartial) {
                    std::cerr << "Failed to allocate memory in BufferedReader";
                    return;
                }
        ...
        if (!fpIn)
        {
            std::cerr << "Unable to open input file: " << szFileName;
            return;
        }
