...
public:
    ...
    virtual void SetInitParams(NV_ENC_INITIALIZE_PARAMS *pParams, NV_ENC_BUFFER_FORMAT eBufferFormat)
    {
        ...
        funcInit(pParams);
        std::cout << NvEncoderInitParam().MainParamToString(pParams);
        std::cout << NvEncoderInitParam().FullParamToString(pParams);
    }
...
private:
    ...
    std::string ConvertValueToString(const std::vector<T> &vValue, const std::string &strValueNames, T value) {
        ...
        if (it == vValue.end()) {
            std::cerr << "Invalid value. Can't convert to one of " << strValueNames;
            return std::string();
        }
        ...
        try {
            if (vQp.size() == 1) {
                ...
                *pQp = {(unsigned)std::stoi(vQp[0]), (unsigned)std::stoi(vQp[1]), (unsigned)std::stoi(vQp[2])};
                ...
                std::cerr << strName << " qp_for_P_B_I or qp_P,qp_B,qp_I (no space is allowed)";
                return false;
            }
        } catch (std::invalid_argument) {
            return false;
